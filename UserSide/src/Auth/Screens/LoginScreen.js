...
const LoginScreen = ({ navigation }) => {
  ...
  const handleSendOTP = async () => {
    ...
    try {
      ...
      const userCheckResult = await clientApi.checkUserExists(phoneNumber);

      if (userCheckResult.success && userCheckResult.data.userExists) {
        // User exists, proceed with OTP
        const otpResult = await clientApi.sendOtp(phoneNumber);

        if (otpResult.success) {
          // Set phone in auth context for OTP verification
